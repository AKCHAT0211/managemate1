...
const Dashboard = () => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:5001/dashboard/stats",
          {
  ...
  const roleBasedStats = {
    Manager: [
      ...
      {
        label: "Projects Not Started",
        value: stats.projectsNotStarted,
  ...
  return (
    <div
      className={`flex flex-col items-center p-8 ml-64 min-h-screen transition-all duration-300 ${
    ...
    >
      ...
      <p
        className={`text-xl mb-4 ${darkMode ? "text-white" : "text-gray-700"}`}
      >
        Role: {user?.role}
      </p>
      ...
      {user?.role === "Project Leader" && (
        <div className="bg-blue-800 text-white p-6 rounded-lg shadow-lg w-3/4 text-center mb-6">
          ...
          <p className="text-gray-300">
            You can view and manage assigned projects.
          </p>
        </div>
      )}
      ...
      <div className="w-full mt-8">
        ...
        <div
          className={`flex flex-col ${
            user?.role === "Team Member" ? "md:flex-row" : ""
